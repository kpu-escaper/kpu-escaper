...
[AddComponentMenu("Arc Reactor Rays/Rays System")]
public class ArcReactor_Arc : MonoBehaviour {
	...
	protected Vector3 CalculateOscillationShift(Vector3 direction, float position, int arcInd)
	{
		...
		foreach (OscillationInfo osc in arcs[arcInd].oscillations)
		{
			...
			{
				float effectiveWavelength = osc.wavelength;
				if (osc.integerPeriods && osc.metric == WavelengthMetric.globalSpace)
					effectiveWavelength = shapeLength/Mathf.Ceil(shapeLength/osc.wavelength);
				if (osc.integerPeriods && osc.metric == WavelengthMetric.localSpace)
					effectiveWavelength = 1/Mathf.Ceil(1/osc.wavelength);
				float angle;
	...
	protected void SetArcShape(int n)
	{
		...
		if (arcs[n].nesting.Nested && !arcs[n].nesting.combinedNesting)
			for (int i = 0; i < segmNums[n] + closeShapeShift; i++)
				arcPoints[n,i] = GetArcPoint((float)i/segmNums[n]*overlapCeof,arcs[n].nesting.parentArcIndex) + shiftVectors[n,i];
			...
			for (int i = 0; i < segmNums[n] + closeShapeShift; i++)
				arcPoints[n,i] = Vector3.Lerp(GetArcPoint((float)i/segmNums[n]*overlapCeof,arcs[n].nesting.parentArcIndex),
				                              ...
				                              arcs[n].nesting.nestingCoef) + shiftVectors[n,i];
		else
			for (int i = 0; i < segmNums[n] + closeShapeShift; i++)
				arcPoints[n,i] = CalcShapePoint((float)i/segmNums[n]*overlapCeof) + shiftVectors[n,i];
	...
	public Vector3 GetArcPoint(float point,int arcIndex)
	{
		...
		return vertices[arcIndex][ind1]*(1-koef) + vert2*koef;
	}
	...
	public Vector3 GetOldArcPoint(float point,int arcIndex)
	{
		...
		return oldVertices[arcIndex][ind1]*(1-koef) + oldVert2*koef;
	}
	...
	protected void Initialize ()
	{
		...
		for(int n = 0; n < arcs.Length; n++)
		{
			//Particle emitter meshes initialization
			for (int q = 0; q < arcs[n].emissionOptions.Length; q++)
			{
				if (arcs[n].emissionOptions[q].emit)
				{
					if (emitterSystems[n][q] == null)
					{
						...
						if (!arcs[n].emissionOptions[q].emitAfterRayDeath)
							...
							partGameObject.AddComponent<ArcReactor_EmitterDestructor>().partSystem = emitterSystems[n][q];
					}
				}
			}
			...
			segmNums[n] = Mathf.Max((int)(shapeLength / arcs[n].sizeOptions.segmentLength)+arcs[n].sizeOptions.minNumberOfSegments,2);			
	...
	void Update()
	{
		...
		if (elapsedTime > lifetime)
		{
			...
			if (playbackType == ArcsPlaybackType.loop)			
				elapsedTime -= lifetime;
			if (playbackType == ArcsPlaybackType.pingpong)
			{
				playBackward = true;
				elapsedTime = lifetime;
			}			
	...
	void LateUpdate () 
	{
		...
		for (int n = 0; n < arcs.Length; n++)
		{
			...
			//Ray size change
			if (arcs[n].sizeOptions.onlyStartWidth)
				lrends[n].SetWidth(arcs[n].sizeOptions.startWidthCurve.Evaluate(lifetimePos),arcs[n].sizeOptions.startWidthCurve.Evaluate(lifetimePos));
			else
				lrends[n].SetWidth(arcs[n].sizeOptions.startWidthCurve.Evaluate(lifetimePos),arcs[n].sizeOptions.endWidthCurve.Evaluate(lifetimePos));
			...
			if (arcs[n].textureOptions.noiseTexture != null)
			{
				...
				if (arcs[n].textureOptions.animateTexture)
				{
					...
					if (noiseOffsets[n]<0)
						noiseOffsets[n] += 1;
					//lrends[n].renderer.material.SetFloat("_NoiseCoef",arcs[n].textureOptions.noisePower);
			...
			for (int i = 0; i < arcs[n].emissionOptions.Length; i++)
			{
				if (arcs[n].emissionOptions[i].emit)
				{
					...
					for (int q = 1; q <= particleCount; q++)
					{
						...
						emitterSystems[n][i].Emit(Vector3.Lerp(emitPos,GetOldArcPoint(rand,n),UnityEngine.Random.value) + randomVect * radius,
