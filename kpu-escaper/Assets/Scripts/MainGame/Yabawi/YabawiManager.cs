...
public class YabawiManager : MainRoomPropertyController{
	...
	public AudioClip Wrong_Yaba;
	public AudioClip Clear_Yaba;
	...
	public override void Clear()
	{
		...
		this.EsAudio.clip = this.Clear_Yaba;
		this.EsAudio.loop = false;

		this.EsAudio.Play ();


		StopCoroutine("RotateObjects");
	...
	void RotateJudge(int j)
	{
		for (int i = 0; i < 16; ++i)
		{
			...
			if (RotateSwitch)
			{
				this.EsAudio.clip = this.RotateSound;
				this.EsAudio.loop = false;
	...
	void OnLeftClick(GameObject obj)
	{
		if(obj.CompareTag("Cube"))
		{
			...
			{
				this.EsAudio.clip = this.Wrong_Yaba;
				this.EsAudio.loop = false;
